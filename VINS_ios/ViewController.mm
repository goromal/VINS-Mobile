...
- (void)viewDidLoad { // MAIN ENTRY POINT <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    [super viewDidLoad];
...
//    }
    ...
    // ENSURE THAT PHONE IS RIGHT TYPE, ELSE CANCEL APP
...
//    featuretracker.vins_pnp.setIMUModel();
    ...
    // ENSURE THAT IOS VERSION IS NEW ENOUGH, ELSE CANCEL APP
...
//        float z_view = (float)vins.correct_Ps[frame_cnt][2];
    ...
    stringView = [NSString stringWithFormat:@"FEATURE: %d",4]; //vins.feature_num];
...
DeviceType deviceName()
{
    ...
    else if(([device compare:@"iPhone11,8"] == NSOrderedSame))
    {
        printf("Device iPhoneXR\n");
